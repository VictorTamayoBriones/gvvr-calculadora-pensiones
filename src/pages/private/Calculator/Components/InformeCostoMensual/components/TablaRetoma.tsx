...
interface TablaRetomaProps {
  generalData: GeneralDataForm;
}

export const TablaRetoma = memo(function TablaRetoma({ generalData }: TablaRetomaProps) {
      ...
      {resultado && resultado.success && resultado.filas && (
        <div className="space-y-3">
          {resultado.advertencias && resultado.advertencias.length > 0 && (
            <div className="space-y-2">
              {resultado.advertencias.map((adv, index) => (
                <div
                  key={`adv-${index}`}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {resultado.filas.map((fila: FilaPago, index: number) => {
          ...
          {resultado.detalles && (
            <div className="text-xs text-muted-foreground space-y-1 mt-3 pt-3 border-t">
              ...
              <p>Precio promedio: {formatMXN(resultado.detalles.precioPromedio)}</p>
            </div>
          )}
        </div>
      )}
